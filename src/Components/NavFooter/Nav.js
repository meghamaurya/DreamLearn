        ...
        <nav className="flex items-center py-1 bg-purple-900 text-white justify-between ">
          <div className="flex px-0 -space-x-0.5 container-fluid ">
            <img
              ...
              className="h-12 rounded-lg cursor-pointer ml-8  px-4 pr-0"
              alt="Dream Learn Logo"
            ></img>
            <span className="font-extrabold flex items-center justify-start text-2xl cursor-pointer mt-3" onClick={handleClick}>
              Dream Learn
            </span>
          </div>
          ...
          <div className="mx-auto max-w-md sm:mx-auto sm:max-w-lg sm:px-10 mt-2 ">
            <form action="" className="relative mx-auto w-max text-white">
              <input type="search"
                className="peer cursor-pointer relative z-10 h-7 w-10 rounded-full bg-transparent pl-8 outline-none focus:w-full focus:cursor-text focus:border-white border-2 focus:pl-16 focus:pr-4 " />
              <svg xmlns="http://www.w3.org/2000/svg" className="absolute inset-y-0 my-auto h-8 w-11 border-r border-transparent stroke-white px-3 peer-focus:border-white peer-focus:stroke-white" fill="none" viewBox="0 0 26 24" stroke="currentColor" strokeWidth="2">
